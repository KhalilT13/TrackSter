...
class QueueReminderManager(context: Context) {
            ...
            .addSnapshotListener { snapshot, error ->
                ...
                snapshot?.documents.orEmpty().forEach { appointment ->
                    ...
                    if (appointment.getString("status") == "waiting") {
                        resolveLabels(appointment) { businessName, serviceName ->
                            appointmentReminders.schedule(id, appointment.getString("appointmentDateKey").orEmpty(),
                                appointment.getLong("appointmentStartMinutes")?.toInt() ?: 0, businessName, serviceName,
                ...
                val id = active.id
                resolveLabels(active) { businessName, _ ->
                    if (active.getString("status") == "in_service") {
                        sendOnce("turn_$id") { notifications.yourTurn(id, businessName) }
                        counterListener?.remove(); counterListener = null; activeAppointmentId = id; return@resolveLabels
                    }
                    if (activeAppointmentId == id && counterListener != null) return@resolveLabels
                    activeAppointmentId = id; counterListener?.remove()
                    val businessId = active.getString("businessId").orEmpty(); val queueNumber = active.getLong("queueNumber")?.toInt() ?: return@resolveLabels
    ...
    private fun resolveLabels(appointment: DocumentSnapshot, callback: (String, String) -> Unit) {
        firestore.collection("businesses").document(appointment.getString("businessId").orEmpty()).get()
