    ...
    private fun startCustomerListener(view: View) {
        ...
        customerListener = firestore.collection(APPOINTMENTS_COLLECTION)
            ...
            .addSnapshotListener { snapshot, error ->
                ...
                val document = snapshot?.documents.orEmpty().filter {
                    it.getString("status") !in listOf(STATUS_COMPLETED, STATUS_CANCELLED)
                }.sortedWith(compareByDescending<com.google.firebase.firestore.DocumentSnapshot> { it.getString("status") == STATUS_IN_SERVICE }
                    .thenBy { it.getTimestamp("createdAt")?.toDate()?.time ?: 0L }).firstOrNull()

                if (document == null) {
                    showEmpty(view)
                } else {
                    val businessId = document.getString("businessId").orEmpty()
                    firestore.collection("businesses").document(businessId).get().addOnCompleteListener { task ->
                        if (!isAdded) return@addOnCompleteListener
                        val business = task.result
                        @Suppress("UNCHECKED_CAST") val legacy = (business?.get("services") as? List<String>).orEmpty()
                        showActive(view, Appointment(
                            id = document.id, businessId = businessId,
                            businessName = business?.getString("businessName").orEmpty().ifBlank { document.getString("businessName").orEmpty() },
                            ...
                            status = document.getString("status").orEmpty(), queueNumber = document.getLong("queueNumber")?.toInt() ?: 0,
                            appointmentDate = document.getString("appointmentDate").orEmpty(), appointmentTime = document.getString("appointmentTime").orEmpty(),
                            createdAtMillis = document.getTimestamp("createdAt")?.toDate()?.time ?: 0L, priceDisplay = document.getString("priceDisplay").orEmpty()
                        ))
                    }
                }
            }
