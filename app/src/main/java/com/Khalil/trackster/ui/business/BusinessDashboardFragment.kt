...
class BusinessDashboardFragment : Fragment(R.layout.fragment_business_dashboard) {
    ...
    private fun startQueueListener(view: View) {
        ...
        queueListener = firestore.collection(APPOINTMENTS_COLLECTION)
            ...
            .addSnapshotListener { snapshot, error ->
                ...
                resolveQueueLabels(view, uid, documents)
            }
    }

    /** Loads current service and customer names instead of trusting copied appointment text. */
    private fun resolveQueueLabels(view: View, businessId: String, documents: List<DocumentSnapshot>) {
        firestore.collection(BUSINESSES_COLLECTION).document(businessId).get().addOnSuccessListener { business ->
            @Suppress("UNCHECKED_CAST") val legacy = (business.get("services") as? List<String>).orEmpty()
            val serviceNames = documents.associate { document ->
                val serviceId = document.getString("serviceId").orEmpty()
                serviceId to AppointmentReferences.serviceName(serviceId, business.get("serviceCatalog"), legacy)
                    .ifBlank { document.getString("serviceType").orEmpty() }
            }
            val customerIds = documents.mapNotNull { it.getString("customerId")?.takeIf(String::isNotBlank) }.distinct()
            val refs = customerIds.map { firestore.collection("publicProfiles").document(it) }
            Tasks.whenAllSuccess<DocumentSnapshot>(refs.map { it.get() }).addOnCompleteListener { profilesTask ->
                if (!isAdded) return@addOnCompleteListener
                val profiles = if (profilesTask.isSuccessful) profilesTask.result.orEmpty() else emptyList()
                val customerNames = profiles.associate { it.id to AppointmentReferences.customerName(it.data) }
                updateInsights(view, documents, serviceNames)
                val entries = documents.map { document ->
                        QueueEntry(
                            id = document.id,
                            queueNumber = document.getLong(FIELD_QUEUE_NUMBER)?.toInt() ?: 0,
                            customerName = customerNames[document.getString("customerId").orEmpty()].orEmpty()
                                .ifBlank { document.getString("customerName").orEmpty() },
                            serviceType = serviceNames[document.getString("serviceId").orEmpty()].orEmpty(),
    ...
    private fun updateInsights(view: View, documents: List<DocumentSnapshot>, serviceNames: Map<String, String>) {
        ...
        val topService = active.groupingBy { serviceNames[it.getString("serviceId").orEmpty()].orEmpty() }.eachCount()
