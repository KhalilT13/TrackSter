...
The repository includes the Firestore indexes and the role-aware Firestore and Storage rules used by the demonstration backend. The submitted live backend is locked to the pre-created lecturer demo accounts; new public accounts cannot access its Firestore or Storage data.

For future rule changes, deploy `firestore.rules`, `firestore.indexes.json`, and `storage.rules` from this repository after replacing the demonstration UID allowlist with the users for your own Firebase project.

If the Firebase CLI is installed and authenticated, deploy the rules from the project root:

    firebase use trackster-c04af
...
## Firestore structure
    ...
    appointments/{appointmentId}
      customerId, customerName, businessId, businessName
...
- The public repository intentionally excludes `local.properties`, `app/google-services.json`, APKs, videos, PDFs, archives, and private lecturer credentials.
